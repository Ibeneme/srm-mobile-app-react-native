...
const Notification: React.FC<NotificationProps> = ({ senderName, message, onPress }) => {
  ...
  const handleNotificationPress = () => {
    onPress(); // Call the onPress handler when the notification is pressed
  };

  const { isDarkModeEnabled, theme } = useTheme();

  const trimmedSenderName = senderName?.length > 26 ? senderName?.substring(0, 26) + '......' : senderName;
